...
char *cmd[]={"mkdir","rmdir","cd","create","open",
            ...
            "help","clear","showbmap","pwd","showinfo","showindex"};
int CMD_len=17;
char command[20];
char* sp;
char dirname[20];
...
{
    int file_flag=0;// File ? Dir ?
    int flag=0;
    if((cmdIdx>=0&&cmdIdx<=8)||cmdIdx==16)
    {
        if(argc==3)
    ...
    int fd=-1;
    if((cmdIdx>=5&&cmdIdx<=7)||cmdIdx==16)//close write read showindex
    {
    ...
    {   
        ...
        case 16:
            ...
            int currentfd=usertable.currentFd;
            char text_tmp[MAX_TEXT_SIZE];
            usertable.openfilelist[currentfd].count=0;
            int r_len = do_read(currentfd,usertable.openfilelist[currentfd].open_inode.length,text_tmp);
            inode* text_inode=(inode*)text_tmp;
            int tmp_flag=-1;
            for(int i=0;i<r_len/sizeof(inode);i++,text_inode++)
            {
                if(file_flag==0)// dir
                {
                    if(!memcmp(text_inode->filename,dirname,strlen(dirname)))
                    {
                        tmp_flag=1;
                        break;           
                    } 
                }else if(file_flag==1)// file
                {
                    if(!memcmp(text_inode->filename,fname,strlen(fname))&&!memcmp(text_inode->exname,exname,3))
                    {
                        tmp_flag=1;
                        break;
                    }
            ...
            if(tmp_flag==-1)
            {
                printRed("no such file or dir");
                return 0;
            }
            ushort* addrs=text_inode->addrs;
            showThreeLevelIndex(addrs);
            break;
        default:
